...
const LocaleZhCN = {
  "values": {
    ...
    "desc.support": "这个页面包含了 MaoXian 的运行信息、当前配置信息，这些信息有利于找出问题或者反馈 bug",
    ...
    "welcome.notice": "<strong>注意：</strong> 如果你要裁剪的页面是在安装本扩展之前打开的，请先刷新那个页面",
    ...
    "setting.path.download.intro": "<strong>下载路径</strong>： 该路径即是你浏览器设置的下载目录，如果你使用「本地程序」的话，该路径是你配置文件（config.yaml）里面的 data_dir 项所配置的目录。",
    "setting.path.filename.intro": "<strong>主文件名</strong>： 即保存下来的主文件的名字，默认为 <code>index.html</code> 或 <code>index.md</code>",
    "setting.folder.root.intro": "<strong>根目录</strong>： 该目录为扩展的存储入口，所有裁剪下来的文件都会存储在该目录下。",
    "setting.folder.category.intro": "<strong>分类目录</strong>： 该目录是用于归类的，可由多级组成，其值由表单的输入决定，例如表单的输入项为 <code>新闻/体育</code> 那么会生成 <code>新闻</code> 这个目录，同时在新闻目录下生成 <code>体育</code> 这个子目录。",
    "setting.folder.clipping.intro": "<strong>裁剪目录</strong>： 扩展可以为每次裁剪都生成一个裁剪目录，并把裁剪下来的文件保存在该目录下",
    ...
    "history.th.handler": "程序",
    "history.th.clip-id": "编号",
    ...
    "reset.current-storage-path-value": "<code>下载目录</code> / <code>$ROOT-FOLDER</code>，其中 <code>$ROOT-FOLDER</code> 为根目录，如果该值和你要选择的目录名字不一致，请到 <a href='go.page:extPage.setting#setting-storage' target='_blank'>设置 &gt; 存储</a> 修改根目录的值，再进行重置。",
